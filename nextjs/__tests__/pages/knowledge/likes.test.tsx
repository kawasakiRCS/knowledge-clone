...
describe('KnowledgeLikesPage', () => {
  ...
  describe('旧システム互換性', () => {
    ...
    test('FontAwesomeアイコンが正しく表示される', async () => {
      ...
      await waitFor(() => {
        const userIcon = screen.getByText('テストユーザー').previousElementSibling;
        expect(userIcon).toHaveClass('fa', 'fa-user');
        const calendarIcon = screen.getByText(/2025-01-11/).previousElementSibling;
        expect(calendarIcon).toHaveClass('fa', 'fa-calendar');
      });
    });
  });
});